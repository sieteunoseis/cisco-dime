...
process.env.NODE_TLS_REJECT_UNAUTHORIZED = "0"; // uncomment if you are using self-signed certificates
...
class ucSoapGetSession {
  ...
  async getOneFileResponse(host, file, callback) {
...
class ucSoapSelectSession {
  ...
  selectLogFilesResponse(host, servicelog, todate, fromdate, timezone, callback) {
    var XML = util.format(
      SELECT_XML_ENVELOPE,
      servicelog,
      todate,
...
class ucSoapListSession {
  ...
  listNodeServiceLogsResponse(host,callback) {
