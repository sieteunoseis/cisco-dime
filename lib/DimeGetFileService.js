...
function clearCookies(host) {
  for (var key of Object.keys(_cookieJar)) {
...
function classifyHttpError(status, statusText, host) {
  ...
  return new errors.DimeError("HTTP " + status + ": " + statusText, {
    host: host,
    statusCode: status,
  });
}
...
async function fetchWithRetry(url, options, config, host) {
  ...
  var retries =
    config.retries != null ? config.retries : DEFAULT_CONFIG.retries;
  ...
  for (var attempt = 0; attempt <= retries; attempt++) {
    ...
    try {
      ...
      if (!response.ok) {
        // Auth errors: clear stale cookies and retry once
        if (response.status === 401 || response.status === 403) {
          if (attempt < retries) {
            debug(
              "auth error %d, clearing cookies and retrying for %s",
              response.status,
              host,
            );
            clearCookies(host);
            var authDelay = retryDelay * Math.pow(2, attempt);
      ...
      if (error instanceof errors.DimeError) {
        ...
        lastError = new errors.DimeError(error.message || String(error), {
          host: host,
        });
      }
...
class ucSoapGetSession {
  ...
  async getOneFile(host, file, callback, onProgress) {
    ...
    var url =
      "https://" +
      host +
      ":8443/logcollectionservice/services/DimeGetFileService";
...
class ucSoapSelectSession {
  ...
  async selectLogFiles(host, servicelog, fromdate, todate, timezone, callback) {
    var XML = SELECT_XML_ENVELOPE.replace("%SERVICELOG%", escapeXml(servicelog))
...
class ucSoapListSession {
  ...
  async listNodeServiceLogs(host, callback) {
    ...
    try {
      ...
      payload.data = buffer;
      debug(
        "listNodeServiceLogs on %s complete (%d bytes)",
        host,
        buffer.length,
      );
      callback(null, payload);
